...
export default function Sharpe(){
  ...
  async function handleAvatarUpload(file){
    ...
    // Try 'avatars' bucket first, then 'public' bucket
    let uploadError, publicUrl;
    for(const bucket of ["avatars","public","images"]){
      const{error,data}=await supabase.storage.from(bucket).upload(filename,file,{upsert:true,contentType:mime});
      if(!error){
    ...
    if(!publicUrl){
      toast2("Upload failed — check Supabase storage buckets");

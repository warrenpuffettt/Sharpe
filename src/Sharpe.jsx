...
export default function Sharpe(){
  ...
  async function handleReceiptUpload(){
    if(!currentUser||!receiptUploadFile) return;
    setReceiptUploading(true);
    const ext=receiptUploadFile.name.split(".").pop().toLowerCase();
    const mime=(ext==="jpg"||ext==="jpeg")?"image/jpeg":ext==="png"?"image/png":"image/jpeg";
    const filename=currentUser.handle.replace("@","")+"_receipt_"+Date.now()+"."+ext;
    const{error:upErr}=await supabase.storage.from("Journals").upload(filename,receiptUploadFile,{upsert:true,contentType:mime});
    ...
    const{error:dbErr}=await supabase.from("receipts").insert({handle:currentUser.handle,url:urlData.publicUrl,month,created_at:new Date().toISOString()});
    if(dbErr){toast2("Error saving receipt");console.error(dbErr);setReceiptUploading(false);return;}
    // Also update receipt_url on trader for badge display
    await supabase.from("traders").update({receipt_url:urlData.publicUrl}).eq("handle",currentUser.handle);
    setReceiptUploading(false);setReceiptUploadFile(null);setReceiptMonth("");setShowReceiptModal(false);
    await fetchProfileData(currentUser.handle);
    const{data:me}=await supabase.from("traders").select("*").eq("handle",currentUser.handle).single();
    if(me){setCurrentUser(me);setProfileTrader(me);}
    await fetchTraders();
    toast2("Receipt uploaded ✓");
  }
          ...
          <div style={{maxWidth:680,margin:"0 auto",padding:"0 24px"}}>
            ...
            {profileTab==="receipts"&&(
              isLoggedIn?(
                <div>
                  {isMe&&(
                    <button onClick={()=>setShowReceiptModal(true)} style={{...s.btnPrimary,marginBottom:16}}>+ Upload Performance Verification</button>
                  )}
                  {profileReceipts.length===0?(
                    <div style={{...s.card,textAlign:"center",padding:48}}>
                      <div style={{fontSize:32,marginBottom:12}}>🧾</div>
                      <div style={{fontSize:14,color:"#555570",marginBottom:4}}>No performance verifications yet</div>
                      <div style={{fontSize:12,color:"#333348"}}>{isMe?"Upload your first performance card to build credibility":"This trader hasn't uploaded verification yet"}</div>
                    </div>
                  ):(
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12}}>
                      {profileReceipts.map((r,i)=>(
                        <div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden"}}>
                          <img src={r.url} alt={r.month} onClick={()=>setLightboxImg(r.url)} style={{width:"100%",display:"block",aspectRatio:"4/3",objectFit:"cover",cursor:"zoom-in"}}/>
                          <div style={{padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <span style={{fontSize:12,color:"#C8A96E",fontFamily:"'DM Mono',monospace",fontWeight:700}}>{r.month||"Performance"}</span>
                            <span style={{fontSize:10,color:"#444458"}}>✓ Verified</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{...s.card,textAlign:"center",padding:32}}>
                  <div style={{fontSize:14,color:"#555570",marginBottom:12}}>Sign in to see performance verifications</div>
        ...
        {lightboxImg&&(
          <div onClick={()=>setLightboxImg(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.96)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:24,cursor:"zoom-out"}}>
            <div style={{position:"relative",maxWidth:"90vw",maxHeight:"90vh"}}>
              ...
              <button onClick={()=>setLightboxImg(null)} style={{position:"absolute",top:-14,right:-14,background:"#1A1A2E",border:"1px solid rgba(255,255,255,0.2)",color:"#E8E8F0",borderRadius:"50%",width:30,height:30,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit",flexShrink:0}}>X</button>
            </div>
          </div>
        )}
        {showReceiptModal&&(
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
            <div style={{background:"#13131A",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:28,maxWidth:480,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                <div style={s.formEyebrow}>PERFORMANCE VERIFICATION</div>
                <button onClick={()=>setShowReceiptModal(false)} style={{background:"none",border:"none",color:"#666680",cursor:"pointer",fontSize:22,fontFamily:"inherit",lineHeight:1}}>✕</button>
              </div>
              <h3 style={{fontSize:18,fontWeight:800,color:"#E8E8F0",margin:"0 0 6px",letterSpacing:"-0.01em"}}>Upload a performance card</h3>
              <p style={{fontSize:13,color:"#555570",marginBottom:20,lineHeight:1.65}}>Help followers verify your track record. Builds trust and credibility on your profile.</p>
              <div style={{background:"rgba(111,207,151,0.05)",border:"1px solid rgba(111,207,151,0.2)",borderRadius:10,padding:14,marginBottom:14}}>
                <div style={{fontSize:11,fontWeight:700,color:"#6FCF97",marginBottom:10,fontFamily:"'DM Mono',monospace",letterSpacing:"0.1em"}}>✓ WHAT TO UPLOAD</div>
                {[["Monthly performance summary","Webull, Robinhood, TD monthly card — shows % return"],["Trade history screenshot","Closed trades with % gain/loss per trade"],["PnL overview","Cropped to % returns only — never show account balance"]].map(([title,sub])=>(
                  <div key={title} style={{marginBottom:8}}><div style={{fontSize:12,color:"#E8E8F0",fontWeight:600}}>{title}</div><div style={{fontSize:11,color:"#555570",marginTop:1}}>{sub}</div></div>
                ))}
              </div>
              <div style={{background:"rgba(235,87,87,0.05)",border:"1px solid rgba(235,87,87,0.2)",borderRadius:10,padding:14,marginBottom:20}}>
                <div style={{fontSize:11,fontWeight:700,color:"#EB5757",marginBottom:10,fontFamily:"'DM Mono',monospace",letterSpacing:"0.1em"}}>✕ NEVER INCLUDE</div>
                {["Account balance in dollars","Full name or personal details","Account or login information","Social security or tax info"].map(item=>(
                  <div key={item} style={{fontSize:12,color:"#888899",marginBottom:5,display:"flex",gap:6}}><span style={{color:"#EB5757",flexShrink:0}}>—</span>{item}</div>
                ))}
              </div>
              ...
              <div style={{marginBottom:20}}>
                <label style={s.label}>Performance Screenshot *</label>
                <input type="file" accept="image/*" style={{...s.input,padding:8}} onChange={e=>setReceiptUploadFile(e.target.files[0])}/>
                {receiptUploadFile&&<div style={{fontSize:11,color:"#6FCF97",marginTop:6}}>{"Ready: "+receiptUploadFile.name}</div>}
              </div>
...
const s={
  ...
  card:{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:18},
  ...
  profileCard:{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:20},
};
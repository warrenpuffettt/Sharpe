...
function isYest(a,b){
  const y=new Date(b);y.setUTCDate(y.getUTCDate()-1);return sameDay(a,y);
}
...
function ScoreHistoryCard({history,verifiedUpdates,totalUpdates}){
  if(!history||history.length===0) return(
    <div style={{...s.card,marginBottom:12,borderColor:"rgba(200,169,110,0.15)"}}>
      <div style={s.cardLabel}>📈 SHARPE SCORE HISTORY</div>
      <div style={{textAlign:"center",padding:"24px 0",color:"#444458",fontSize:13}}>Update your stats to start building your score history</div>
    </div>
  );
  ...
  const verifyRate=totalUpdates>0?Math.round((verifiedUpdates/totalUpdates)*100):0;
  return(
    <div style={{...s.card,marginBottom:12,borderColor:"rgba(200,169,110,0.15)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
        <div>
          <div style={s.cardLabel}>📈 SHARPE SCORE HISTORY</div>
          <div style={{fontSize:11,color:"#555570",marginTop:2}}>Your trading credit score over time</div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:28,fontWeight:900,color:"#C8A96E",lineHeight:1}}>{latest}</div>
          <div style={{fontSize:11,color:trend>=0?"#6FCF97":"#EB5757",fontWeight:700,marginTop:2}}>{trend>=0?"↑+":"↓"}{Math.abs(trend)} pts all time</div>
        </div>
      </div>

      {/* Chart */}
      <ScoreChart data={history} width={280} height={80}/>

      {/* Stats row */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginTop:14,paddingTop:14,borderTop:"1px solid rgba(255,255,255,0.05)"}}>
        {[
          {v:highest,l:"Peak",c:"#C8A96E"},
          {v:lowest,l:"Low",c:"#888899"},
          {v:totalUpdates||0,l:"Updates",c:"#E8E8F0"},
          {v:verifyRate+"%",l:"Verified",c:"#6FCF97"},
        ].map((stat,i)=>(
          <div key={i} style={{textAlign:"center"}}>
            <div style={{fontSize:15,fontWeight:700,color:stat.c}}>{stat.v}</div>
            <div style={{fontSize:9,color:"#444458",fontFamily:"'DM Mono',monospace",marginTop:2}}>{stat.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{marginTop:12}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
          <span style={{fontSize:10,color:"#555570",fontFamily:"'DM Mono',monospace"}}>AI VERIFICATION RATE</span>
          <span style={{fontSize:10,color:"#6FCF97",fontFamily:"'DM Mono',monospace",fontWeight:700}}>{verifyRate}%</span>
        </div>
        <div style={{height:4,background:"rgba(255,255,255,0.05)",borderRadius:2,overflow:"hidden"}}>
          <div style={{height:"100%",width:verifyRate+"%",background:"linear-gradient(90deg,#6FCF97,#4CAF7D)",borderRadius:2,transition:"width .5s"}}/>
        </div>
        <div style={{fontSize:10,color:"#333348",marginTop:4}}>{verifiedUpdates||0} of {totalUpdates||0} updates verified via brokerage screenshot</div>
      </div>
    </div>
  );
}
...
function ShareCard({trader,rank,onClose}){
  const cardRef = useRef();
  const score=calcScore(trader.monthly_return,trader.win_rate,trader.trade_count,trader.max_drawdown);
  const tier=getTier(score);
  const accent=trader.accent_color||"#C8A96E";
  const profileUrl="sharpe-eta.vercel.app/u/"+trader.handle.replace("@","");

  function copyLink(){
    navigator.clipboard.writeText("https://"+profileUrl);
    // Brief visual feedback handled by toast in parent
  }

  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.95)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20,flexDirection:"column",gap:16}}>
      {/* The card itself - this is what they screenshot */}
      <div ref={cardRef} style={{background:"linear-gradient(135deg,#0D0D18 0%,#13131F 60%,#0A0A14 100%)",border:"1px solid "+accent+"44",borderRadius:20,padding:32,width:360,position:"relative",overflow:"hidden",boxShadow:"0 0 80px "+accent+"22"}}>
        {/* Background score watermark */}
        <div style={{position:"absolute",top:-10,right:-10,fontSize:140,fontWeight:900,color:accent,opacity:0.04,lineHeight:1,pointerEvents:"none",userSelect:"none"}}>{score}</div>

        {/* Header */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:13,fontWeight:700,letterSpacing:"0.18em",color:accent}}>SHARPE</div>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"rgba(255,255,255,0.3)",letterSpacing:"0.1em"}}>#{rank} RANKED</div>
        </div>

        {/* Avatar + handle */}
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
          {trader.avatar_url
            ?<img src={trader.avatar_url} style={{width:56,height:56,borderRadius:"50%",objectFit:"cover",border:"2px solid "+accent+"66"}} alt=""/>
            :<div style={{width:56,height:56,borderRadius:"50%",background:accent+"22",border:"2px solid "+accent+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:800,color:accent}}>{trader.handle.replace("@","").slice(0,2).toUpperCase()}</div>
          }
          <div>
            <div style={{fontSize:18,fontWeight:800,color:"#E8E8F0",letterSpacing:"-0.01em"}}>{trader.handle}</div>
            <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
              {showBadge(score)&&<span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:20,fontFamily:"'DM Mono',monospace",letterSpacing:"0.08em",color:tier.color,background:tier.bg,border:"1px solid "+tier.color+"44"}}>{tier.label}</span>}
              {trader.trading_style&&<span style={{fontSize:10,fontWeight:600,padding:"2px 7px",borderRadius:4,background:"rgba(255,255,255,0.06)",color:"#888899",fontFamily:"'DM Mono',monospace"}}>{trader.trading_style}</span>}
              {trader.x_verified&&<span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:4,background:"rgba(29,161,242,0.1)",color:"#1DA1F2",fontFamily:"'DM Mono',monospace"}}>𝕏 Verified</span>}
            </div>
          </div>
        </div>
        ...
        <div style={{marginBottom:20}}>
          <div style={{fontSize:72,fontWeight:900,color:accent,lineHeight:1,textShadow:"0 0 40px "+accent+"66"}}>{score}</div>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:accent,letterSpacing:"0.18em",opacity:0.6,marginTop:2}}>SHARPE SCORE</div>
        </div>

        {/* Stats row */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:20,background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"12px 10px"}}>
          {[
            {v:(trader.monthly_return>0?"+":"")+trader.monthly_return+"%",l:"Return",c:trader.monthly_return>=0?"#6FCF97":"#EB5757"},
            {v:trader.win_rate+"%",l:"Win Rate",c:"#E8E8F0"},
            {v:trader.trade_count,l:"Trades",c:"#E8E8F0"},
            {v:"🔥"+trader.streak+"d",l:"Streak",c:trader.streak>=7?accent:"#E8E8F0"},
          ].map((stat,i)=>(
            <div key={i} style={{textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:700,color:stat.c,lineHeight:1.2}}>{stat.v}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.3)",marginTop:3,fontFamily:"'DM Mono',monospace"}}>{stat.l}</div>
            </div>
          ))}
        </div>

        {/* Verification strip */}
        {(trader.verified_updates||0)>0&&(
          <div style={{background:"rgba(111,207,151,0.06)",border:"1px solid rgba(111,207,151,0.15)",borderRadius:8,padding:"8px 12px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:14}}>✓</span>
            <div style={{fontSize:11,color:"#6FCF97",fontFamily:"'DM Mono',monospace"}}>{trader.verified_updates} AI-VERIFIED UPDATES · BROKERAGE CONFIRMED</div>
          </div>
        )}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:14}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(255,255,255,0.25)",letterSpacing:"0.1em"}}>{profileUrl}</div>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:accent,letterSpacing:"0.08em"}}>sharpe</div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap",justifyContent:"center"}}>
        <button
          ...
          style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",color:"#E8E8F0",padding:"10px 20px",borderRadius:8,cursor:"pointer",fontSize:13,fontFamily:"inherit",fontWeight:600}}>
          🔗 Copy Profile Link
        </button>
        <div style={{background:"rgba(200,169,110,0.1)",border:"1px solid rgba(200,169,110,0.3)",color:"#C8A96E",padding:"10px 20px",borderRadius:8,fontSize:13,fontFamily:"inherit",fontWeight:600,display:"flex",alignItems:"center",gap:6}}>
          📸 Screenshot this card to share
        </div>
        <button onClick={onClose} style={{background:"transparent",border:"1px solid rgba(255,255,255,0.1)",color:"#666680",padding:"10px 20px",borderRadius:8,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Close</button>
      </div>
      <div style={{fontSize:12,color:"#333348",textAlign:"center"}}>Screenshot the card above and post it on 𝕏 — or copy your profile link to share directly</div>
    </div>
  );
}
...
export default function Sharpe(){
  ...
  const [joinForm,setJoinForm]=useState({email:"",password:"",handle:"",bio:"",tradingStyle:"",monthlyReturn:"",winRate:"",tradeCount:"",maxDrawdown:"",inviteCode:"",whopUrl:""});
  ...
  const [pendingProfileHandle,setPendingProfileHandle]=useState(null);
  ...
  useEffect(()=>{
    fetchAll();
    // Check URL for direct profile link e.g. /u/warrenpuffettt
    const path=window.location.pathname;
    const match=path.match(/^\/u\/(.+)$/);
    if(match){
      const handle="@"+decodeURIComponent(match[1]).replace(/^@/,"");
      // Will open profile once traders load
  ...
  async function fetchAll(){
    setLoading(true);
    await Promise.all([fetchTraders(),fetchFeed(),fetchComments()]);
    setLoading(false);
  }
  // Show loading spinner on initial load
  // Loading spinner rendered inline in views, not as early return

  useEffect(()=>{
    if(pendingProfileHandle&&traders.length>0){
      const trader=traders.find(t=>t.handle.toLowerCase()===pendingProfileHandle.toLowerCase());
      if(trader){openProfile(trader);setPendingProfileHandle(null);}
    }
  },[traders,pendingProfileHandle]);
  ...
  function complete(id){/* streak/challenge tracking */}
  ...
  async function handleAIConfirm(data){
    ...
    if(last){
      if(sameDay(last,today)){
        streak=currentUser.streak; // already updated today
      } else if(isYest(last,today)){
        streak=(currentUser.streak||0)+1; // consecutive day
      } else {
        streak=1; // streak broken
      }
    } else {
      streak=1; // first update
    }
    ...
    if(error){toast2("Error updating — "+error.message);console.error(error);return;}
    // Re-fetch to get full updated record including streak
    const{data:fresh}=await supabase.from("traders").select("*").eq("handle",currentUser.handle).single();
  ...
  async function handleSignUp(){
    ...
    const{data,error}=await supabase.from("traders").upsert({
      auth_id:authData.user.id,handle:h,bio,trading_style:tradingStyle,avatar_url:avatarUrl,banner_url:bannerUrl,whop_url:joinForm.whopUrl||null,
  ...
  async function signOut(){await supabase.auth.signOut();setCurrentUser(null);setAuthUser(null);window.history.pushState({},"","/");setView("landing");}
  function navTo(v){window.history.pushState({},"","/");setView(v);}
  ...
  const Nav=()=>(
    <nav style={s.nav}>
      <span onClick={()=>{window.history.pushState({},"","/");setView("landing");}} style={{...s.logo,cursor:"pointer"}}>SHARPE</span>
  ...
  // Loading spinner - safe here, all hooks declared above
  if(loading&&traders.length===0&&!currentUser) return(
    <div style={{...s.page,display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:20,fontWeight:700,letterSpacing:"0.18em",color:"#C8A96E",marginBottom:16}}>SHARPE</div>
        <div style={{display:"flex",justifyContent:"center",gap:6}}>
          {[0,1,2].map(i=>(<div key={i} style={{width:8,height:8,borderRadius:"50%",background:"#C8A96E",opacity:0.4}}/>))}
        </div>
      </div>
    </div>
  );
  ...
  if(view==="join") return(
    <div style={s.page}><div style={s.grain}/><Nav/>
      <div style={s.formWrap}><div style={s.formCard}>
        ...
        <div style={s.fg}><label style={s.label}>Whop Alerts Link (Optional)</label><input style={s.input} placeholder="https://whop.com/your-alerts" value={joinForm.whopUrl} onChange={e=>setJoinForm({...joinForm,whopUrl:e.target.value})}/><div style={{fontSize:11,color:"#444458",marginTop:4}}>Add later from your profile settings</div></div>
        <div style={s.divider}/>
    ...
    <div style={s.page}><div style={s.grain}/><Nav/>
      <div style={{maxWidth:900,margin:"0 auto",padding:"0 24px"}}>
        <div style={{paddingTop:60,paddingBottom:48,textAlign:"center"}}>
          ...
          <p style={{fontSize:17,color:"#666680",lineHeight:1.75,maxWidth:520,margin:"0 auto 36px"}}>Sharpe verifies your performance, builds your credibility, and helps you monetize your edge — so you can focus on trading. Think of us as your agent.</p>
              ...
              {top3.map((trader,i)=>{
                ...
                return(
                  <div key={trader.handle} onClick={()=>openProfile(trader)} style={{background:isFirst?"rgba(200,169,110,0.07)":"rgba(255,255,255,0.02)",border:"1px solid "+(isFirst?"rgba(200,169,110,0.3)":"rgba(255,255,255,0.07)"),borderRadius:16,padding:24,textAlign:"center",cursor:"pointer",boxShadow:isFirst?"0 0 40px rgba(200,169,110,0.08)":"none"}}>
                    ...
                    {(trader.verified_updates||0)>0&&<div style={{fontSize:11,color:"#6FCF97",marginTop:8}}>✓ {trader.verified_updates} AI Verified</div>}
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:56,paddingBottom:56,textAlign:"center"}}>
          ...
          <h3 style={{fontSize:32,fontWeight:800,color:"#E8E8F0",marginBottom:14,letterSpacing:"-0.02em"}}>We are your <span style={{color:"#C8A96E"}}>agent.</span></h3>
          <p style={{color:"#666680",fontSize:15,maxWidth:480,margin:"0 auto 32px",lineHeight:1.75}}>Think of Sharpe like a sports agent. We find traders with real skill, verify their performance so their audience trusts them, and handle the business side so they can focus on what they do best — trading. We grow together.</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,maxWidth:700,margin:"0 auto 32px",textAlign:"left"}}>
            {[
              {icon:"📊",title:"We verify your performance",desc:"AI-powered screenshot analysis and update history build a track record your audience can trust."},
              {icon:"💰",title:"We handle monetization",desc:"Hit score 70+ and we set you up with a paid alert subscription through Whop. You focus on trading."},
              {icon:"🌐",title:"We bring you new subscribers",desc:"Traders on the leaderboard get exposure to audiences they would never reach through their own X following alone."},
              {icon:"🔒",title:"Your credibility stays yours",desc:"Your verified track record, score history, and reputation live on Sharpe. That is infrastructure most traders spend years building."},
            ].map((item,i)=>(
              <div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,padding:20}}>
                <div style={{fontSize:24,marginBottom:10}}>{item.icon}</div>
                <div style={{fontSize:14,fontWeight:700,color:"#E8E8F0",marginBottom:6}}>{item.title}</div>
                <div style={{fontSize:13,color:"#666680",lineHeight:1.65}}>{item.desc}</div>
              </div>
            ))}
          </div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:48,paddingBottom:64,textAlign:"center"}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,letterSpacing:"0.2em",color:"#555570",marginBottom:16}}>TOP PERFORMERS GET NOTICED</div>
          <h3 style={{fontSize:28,fontWeight:800,color:"#E8E8F0",marginBottom:14,letterSpacing:"-0.02em"}}>The leaderboard has <span style={{color:"#C8A96E"}}>real stakes.</span></h3>
          <p style={{color:"#666680",fontSize:15,maxWidth:480,margin:"0 auto 32px",lineHeight:1.75}}>Top ranked traders get personally reached out to about monetization opportunities. Your rank is not just a number — it is how we find you.</p>
          <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
            {[{n:"Score 70+",l:"Alerts unlocked"},{n:"Top 10",l:"Personal outreach"},{n:"#1",l:"Featured on landing page"}].map((item,i)=>(
              <div key={i} style={{padding:"16px 24px",border:"1px solid rgba(200,169,110,0.2)",borderRadius:10,background:"rgba(200,169,110,0.04)",textAlign:"center",minWidth:140}}>
                <div style={{fontSize:20,fontWeight:800,color:"#C8A96E"}}>{item.n}</div>
                <div style={{fontSize:12,color:"#666680",marginTop:4}}>{item.l}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  if(view==="dashboard"&&currentUser){
    ...
    return(
      <div style={s.page}><div style={s.grain}/><Nav/>
        ...
        <div style={{maxWidth:980,margin:"0 auto",padding:"20px 16px",display:"grid",gridTemplateColumns:window.innerWidth>700?"1fr 300px":"1fr",gap:14,position:"relative",zIndex:1}}>
          <div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
                    <div>
                      ...
                      <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:4}}>
                        ...
                        {(currentUser.verified_updates||0)>0&&<span style={{...s.styleTag,background:"rgba(111,207,151,0.1)",color:"#6FCF97"}}>✓ {currentUser.verified_updates} Verified</span>}
            ...
            {canAlerts&&<div style={{background:"rgba(111,207,151,0.05)",border:"1px solid rgba(111,207,151,0.2)",borderRadius:10,padding:12,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}><div><div style={{fontSize:13,color:"#6FCF97",fontWeight:700}}>🔔 Alerts Unlocked — Score {score}</div><div style={{fontSize:12,color:"#555570",marginTop:2}}>DM @warrenpuffettt on X to get set up.</div></div><a href={"https://x.com/messages/compose?recipient_id=warrenpuffettt&text=Hey+I+hit+score+"+String(score)+"+on+Sharpe+and+want+to+set+up+alerts"} target="_blank" rel="noopener noreferrer" style={{...s.chip,color:"#6FCF97",borderColor:"rgba(111,207,151,0.3)",textDecoration:"none",flexShrink:0}}>DM on 𝕏 →</a></div>}
          ...
          <div>
            <ScoreHistoryCard history={currentUser.score_history||[]} verifiedUpdates={currentUser.verified_updates||0} totalUpdates={currentUser.total_updates||0}/>
  ...
  if(view==="alerts"&&currentUser){
    const score=calcScore(currentUser.monthly_return,currentUser.win_rate,currentUser.trade_count,currentUser.max_drawdown);
    const eligible=score>=ALERT_MIN;
    return(
      <div style={s.page}><div style={s.grain}/><Nav/>
        <div style={s.formWrap}><div style={s.formCard}>
          <div style={s.formEyebrow}>SHARPE ALERTS</div>
          <h2 style={s.formTitle}>{eligible?"You're in 🎉":"Keep climbing"}</h2>
          {eligible?(
            <>
              <div style={{background:"linear-gradient(135deg,rgba(200,169,110,0.1),rgba(200,169,110,0.04))",border:"1px solid rgba(200,169,110,0.25)",borderRadius:14,padding:24,textAlign:"center",marginBottom:20}}>
                <div style={{fontSize:52,marginBottom:8}}>🔔</div>
                <div style={{fontSize:22,fontWeight:800,color:"#C8A96E",marginBottom:6}}>Alerts Unlocked</div>
                <div style={{fontSize:14,color:"#888899",lineHeight:1.7,maxWidth:360,margin:"0 auto"}}>Your Sharpe Score of <span style={{color:"#C8A96E",fontWeight:700}}>{score}</span> qualifies you to offer paid alerts through Sharpe. DM us on X to get set up — we handle everything through Whop and you keep the majority of every subscription.</div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:24}}>
                {[{n:"You set",l:"your price"},{n:"Fast",l:"setup via Whop"},{n:"Your cut",l:"majority to you"}].map((item,i)=>(
                  <div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"14px 10px",textAlign:"center"}}>
                    <div style={{fontSize:16,fontWeight:800,color:"#C8A96E"}}>{item.n}</div>
                    <div style={{fontSize:11,color:"#555570",marginTop:3}}>{item.l}</div>
                  </div>
                ))}
              </div>
              <a href={"https://x.com/messages/compose?recipient_id=warrenpuffettt&text=Hey%2C+I+hit+score+"+String(score)+"+on+Sharpe+and+want+to+set+up+alerts+for+my+followers."} target="_blank" rel="noopener noreferrer" style={{display:"block",background:"#C8A96E",color:"#0A0A0F",border:"none",padding:"14px 24px",borderRadius:8,cursor:"pointer",fontSize:14,fontWeight:700,fontFamily:"inherit",width:"100%",textAlign:"center",textDecoration:"none",boxSizing:"border-box"}}>
                𝕏 DM @warrenpuffettt to Get Started →
              </a>
              <div style={{fontSize:12,color:"#444458",textAlign:"center",marginTop:10}}>We will get you set up within 24 hours</div>
            </>
          ):(
            <div style={{textAlign:"center",padding:"20px 0"}}>
              <div style={{fontSize:52,marginBottom:12}}>🔒</div>
              <div style={{fontSize:15,color:"#888899",marginBottom:6}}>You need a Sharpe Score of <span style={{color:"#C8A96E",fontWeight:700}}>70</span> to unlock alerts</div>
              <div style={{fontSize:13,color:"#555570",marginBottom:20}}>You are at <span style={{color:"#C8A96E",fontWeight:700}}>{score}</span> — {ALERT_MIN-score} more points to go</div>
              <ScoreBar score={score}/>
              <div style={{fontSize:12,color:"#444458",marginTop:8}}>{score} / {ALERT_MIN}</div>
            </div>
          )}
        </div></div>
        {toast&&<Toast msg={toast}/>}
    ...
    <div style={s.page}><div style={s.grain}/><Nav/>
      <div style={{maxWidth:680,margin:"0 auto",padding:"32px 24px",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          {visibleFeed.length===0&&<div style={{textAlign:"center",color:"#444458",padding:60}}><div style={{fontSize:32,marginBottom:12}}>📭</div><div style={{fontSize:14}}>No posts yet{feedScope==="following"?" from traders you follow":""}.</div>{!currentUser&&<div style={{fontSize:12,marginTop:8}}><button onClick={()=>setView("join")} style={{...s.btnPrimarySmall,marginTop:8}}>Join to post</button></div>}</div>}
        </div>
      </div>
      {toast&&<Toast msg={toast}/>}
    </div>
  ...
  if(view==="profile"&&profileTrader){
    ...
    return(
      <div style={{...s.page,background:"#08080E"}}><div style={s.grain}/><Nav/>
          ...
          <div style={{maxWidth:680,margin:"0 auto",padding:"0 24px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginTop:-40,marginBottom:20,flexWrap:"wrap",gap:12}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <button onClick={()=>setShowShareCard(true)} style={{...s.chip,color:"#C8A96E",borderColor:"rgba(200,169,110,0.3)"}}>📸 Share</button>
              </div>
            </div>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:5}}>
                  ...
                  {profileTrader.audited&&<span style={{...s.styleTag,background:"rgba(111,207,151,0.1)",color:"#6FCF97"}}>✓ Audited</span>}
                      ...
                      {profileTrader.whop_url&&<a href={profileTrader.whop_url} target="_blank" rel="noopener noreferrer" style={{...s.styleTag,background:"rgba(111,207,151,0.08)",color:"#6FCF97",textDecoration:"none"}}>🔔 Subscribe on Whop ↗</a>}
                  {(profileTrader.verified_updates||0)>0&&<span style={{...s.styleTag,background:"rgba(111,207,151,0.1)",color:"#6FCF97"}}>✓ {profileTrader.verified_updates} AI Verified</span>}
                </div>
            ...
            {isMe&&(
              <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:12}}>
                <div style={s.card}>
                  <div style={s.cardLabel}>🔗 WHOP ALERTS LINK</div>
                  <input
                    style={{...s.input,fontSize:13}}
                    placeholder="https://whop.com/your-alerts"
                    defaultValue={currentUser?.whop_url||""}
                    onBlur={async e=>{
                      const url=e.target.value.trim();
                      const{data}=await supabase.from("traders").update({whop_url:url||null}).eq("handle",currentUser.handle).select().single();
                      if(data){setCurrentUser(data);setProfileTrader(data);await fetchTraders();toast2("Whop link saved");}
                    }}
                  />
                  <div style={{fontSize:11,color:"#444458",marginTop:6}}>Add your Whop alerts page so followers can subscribe directly from your profile</div>
                </div>
                <div style={s.card}>
            ...
            {isMe&&(
              <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:16}}>
                ...
                <button style={{...s.btnPrimary,background:"linear-gradient(135deg,#1a1a2e,#16213e)",border:"1px solid rgba(200,169,110,0.3)",color:"#C8A96E"}} onClick={()=>setShowShareCard(true)}>📸 Share My Profile Card</button>
        ...
        {/* Share Card */}
        {showShareCard&&profileTrader&&(
          <ShareCard
            trader={profileTrader}
            rank={sorted.findIndex(t=>t.handle===profileTrader.handle)+1}
              ...
              <div style={{display:"flex",gap:14,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:4}}>
                    ...
                    {(king.verified_updates||0)>0&&<span style={{fontSize:11,color:"#6FCF97"}}>✓ {king.verified_updates} verified</span>}
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,flex:1,minWidth:0}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",marginBottom:2}}>
                      ...
                      {(trader.verified_updates||0)>0&&<span style={{...s.styleTag,fontSize:9,background:"rgba(111,207,151,0.1)",color:"#6FCF97"}}>✓{trader.verified_updates}</span>}
...
const s={
  page:{minHeight:"100vh",background:"#09090F",color:"#E8E8F0",fontFamily:"'DM Sans','Helvetica Neue',sans-serif",position:"relative",overflowX:"hidden",WebkitOverflowScrolling:"touch"},
  ...
  nav:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderBottom:"1px solid rgba(255,255,255,0.05)",position:"sticky",top:0,background:"rgba(9,9,15,0.94)",backdropFilter:"blur(16px)",zIndex:50,flexWrap:"wrap",gap:6},
  logo:{fontFamily:"'DM Mono',monospace",fontSize:17,fontWeight:700,letterSpacing:"0.18em",color:"#C8A96E"},
  chip:{background:"transparent",border:"1px solid rgba(255,255,255,0.1)",color:"#888899",padding:"5px 12px",borderRadius:6,cursor:"pointer",fontSize:12,fontFamily:"inherit",display:"flex",alignItems:"center",gap:5,transition:"all .15s",whiteSpace:"nowrap"},
  ...
  formWrap:{maxWidth:500,margin:"0 auto",padding:"32px 16px",position:"relative",zIndex:1},
  ...
  fieldRow:{display:"flex",gap:12,flexWrap:window.innerWidth<500?"wrap":"nowrap"},
  label:{display:"block",fontSize:11,color:"#555570",marginBottom:6,fontFamily:"'DM Mono',monospace",letterSpacing:"0.05em"},
  input:{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"10px 12px",color:"#E8E8F0",fontSize:14,fontFamily:"inherit",outline:"none",boxSizing:"border-box",transition:"border-color .15s"},

      ...
      <div style={{background:"#0F0F18",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:28,maxWidth:500,width:"100%",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        {step==="confirm"&&parsed&&(
          <>
            ...
            <button onClick={()=>onConfirm({...edited,ticker,tradePct,receiptFile:null})} style={{...s.btnPrimary,background:"linear-gradient(135deg,#C8A96E,#E8C97E)",color:"#0A0A0F",marginBottom:8}}>
        ...
        {step==="manual"&&(
          <>
            ...
            <button onClick={()=>onConfirm({...edited,ticker,tradePct,receiptFile:null})} style={{...s.btnPrimary,marginBottom:8}}>Save Stats →</button>
  ...
  if(view==="feed") return(
    <div style={s.page}><div style={s.grain}/><Nav/>
      <div style={{maxWidth:680,margin:"0 auto",padding:"32px 24px",position:"relative",zIndex:1}}>
        ...
        <AdBanner/>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {visibleFeed.map((post,i)=><div key={post.id}><PostCard post={post} currentUser={currentUser} onLike={handleLike} onComment={handleComment} onReact={handleReact} comments={comments} traders={traders} onOpen={openProfile}/>{i>0&&(i+1)%8===0&&<AdBanner/>}</div>)}
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{maxWidth:680,margin:"0 auto",padding:"0 24px"}}>
            ...
            {profileTab==="receipts"&&(
              isLoggedIn?(
                <div>
                  {isMe&&<button onClick={()=>setShowReceiptModal(true)} style={{...s.btnPrimary,marginBottom:14}}>+ Upload Performance Verification</button>}
                  {profileReceipts.length===0?(
                    <div style={{...s.card,textAlign:"center",padding:48}}>
                      ...
                      <div style={{fontSize:12,color:"#333348"}}>{isMe?"Upload your first monthly performance card":"This trader hasn't uploaded verification yet"}</div>
                    </div>
                  ):(
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                            ...
                            {isOpen&&urls.length>0&&(
                              <div style={{padding:"0 14px 14px"}}>
                                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
                                  {urls.map((url,j)=>(
                                    <div key={j} onClick={()=>setLightboxImg(url)} style={{borderRadius:8,overflow:"hidden",cursor:"zoom-in",border:"1px solid rgba(255,255,255,0.06)"}}>
                                      <img src={url} alt={r.month} style={{width:"100%",display:"block",aspectRatio:"4/3",objectFit:"cover"}}/>
                                    </div>
                                  ))}
                                </div>
                              </div>
                ...
                <div style={{...s.card,textAlign:"center",padding:32}}>
                  <div style={{fontSize:14,color:"#555570",marginBottom:12}}>Sign in to see receipts</div>

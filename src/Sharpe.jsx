...
function MiniChart({ data, width = 120 }) {
  if (!data || data.length < 2) return null;
  const max = Math.max(...data);
  const min = Math.min(...data);
  const range = max - min || 1;
  const w = width, h = 40, pad = 4;
  ...
  const color = data[data.length-1] >= data[0] ? "#6FCF97" : "#EB5757";
  return (
...
const ScoreBreakdown = ({ returnPct, winRate, tradeCount, maxDrawdown }) => {
  const score = calcSharpeScore(returnPct, winRate, tradeCount, maxDrawdown);
  return (
    <div style={s.scoreCard}>
      <div style={s.cardLabel}>HOW YOUR SCORE IS CALCULATED</div>
      <div style={s.scoreRows}>
        {[
          { label: "Monthly Return %", weight: "35%", value: returnPct !== undefined ? `${returnPct > 0 ? "+" : ""}${returnPct}%` : "—" },
          { label: "Win Rate %", weight: "35%", value: winRate !== undefined ? `${winRate}%` : "—" },
          { label: "Trade Volume", weight: "15%", value: tradeCount !== undefined ? `${tradeCount} trades` : "—" },
          { label: "Max Drawdown (lower = better)", weight: "15%", value: maxDrawdown !== undefined ? `-${maxDrawdown}%` : "—" },
        ].map((row, i) => (
          <div key={i} style={s.scoreRow}>
            <div style={{ flex: 1 }}>
              <span style={s.scoreRowLabel}>{row.label}</span>
              {row.value !== "—" && <span style={s.scoreRowVal}>{row.value}</span>}
            </div>
...
export default function Sharpe() {
  ...
  const [joinForm, setJoinForm] = useState({ handle: "", monthlyReturn: "", winRate: "", tradeCount: "", maxDrawdown: "", ticker: "", tradePct: "" });
  const [updateForm, setUpdateForm] = useState({ monthlyReturn: "", winRate: "", tradeCount: "", maxDrawdown: "", ticker: "", tradePct: "" });
  ...
  async function fetchCurrentUser(handle) {
    ...
    else { localStorage.removeItem("sharpe_handle"); setView("login"); }
  }
  ...
  async function handleLogin() {
    if (!loginHandle) { showToast("Enter your X handle"); return; }
    ...
    if (data) {
      ...
      showToast("Handle not found. Join first.");
    }
  }
  ...
  async function handleJoin() {
    ...
    const { data, error } = await supabase.from("traders").upsert({
      ...
      max_drawdown: parseFloat(joinForm.maxDrawdown) || 0,
      streak: 1,
  ...
  async function handleUpdate() {
    ...
    const newTrades = updateForm.ticker
      ...
      : currentUser.trades;
    const newHistory = [...(currentUser.history || []), newReturn].slice(-6);
    ...
    setCurrentUser(data);
    await fetchTraders();
    setUpdateForm({ monthlyReturn: "", winRate: "", tradeCount: "", maxDrawdown: "", ticker: "", tradePct: "" });
    setView("dashboard");
    showToast("Stats updated. Leaderboard refreshed.");
  }

  function handleSignOut() {
    localStorage.removeItem("sharpe_handle");
    setCurrentUser(null);
    setView("landing");
  }
    ...
    <div style={s.page}>
      ...
      <div style={s.formWrap}>
        <div style={s.formCard}>
          ...
          <p style={s.formSub}>Enter your X handle to access your dashboard.</p>
          <div style={s.fieldGroup}>
            <label style={s.label}>X Handle</label>
            <input style={s.input} placeholder="@yourhandle" value={loginHandle}
  ...
  if (view === "landing") return (
    <div style={s.page}>
      ...
      <div style={s.section}>
        ...
        <div style={s.steps}>
          {[
            ...
            { n: "02", t: "Submit Daily", d: "Log your running monthly return %, win rate, drawdown, and best trade ticker." },
            { n: "03", t: "Compete", d: "Your Sharpe Score updates live. See where you rank against every trader on the board." },
          ].map(step => (
      ...
      <div style={{ ...s.section, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <p style={s.scoreSub}>One lucky trade won't put you at the top. Sharpe rewards consistent, disciplined traders with controlled risk.</p>
      </div>
      <div style={s.footer}>
        <span style={s.logo}>SHARPE</span>
        ...
        <div style={s.dashWrap}>
          <div style={s.profileCard}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <a href={`https://x.com/${currentUser.handle.replace("@", "")}`} target="_blank" rel="noopener noreferrer" style={{ ...s.profileHandle, textDecoration: "none", color: "#E8E8F0" }}>{currentUser.handle} <span style={{ fontSize: 13, color: "#C8A96E" }}>↗</span></a>
            ...
            {currentUser.max_drawdown > 0 && (
              <div style={{ fontSize: 13, color: "#888", marginBottom: 12 }}>Max Drawdown: <span style={{ color: "#EB5757" }}>-{currentUser.max_drawdown}%</span></div>
            )}
          ...
          {currentUser.history && currentUser.history.length > 1 && (
            <div style={s.card}>
              <div style={s.cardLabel}>RETURN HISTORY</div>
              <MiniChart data={currentUser.history} width={300} />
              <div style={{ display: "flex", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
                {currentUser.history.map((v, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: v >= 0 ? "#6FCF97" : "#EB5757" }}>{v > 0 ? "+" : ""}{v}%</div>
                    <div style={{ fontSize: 10, color: "#555570" }}>Update {i + 1}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <ScoreBreakdown returnPct={currentUser.monthly_return} winRate={currentUser.win_rate} tradeCount={currentUser.trade_count} maxDrawdown={currentUser.max_drawdown} />
      ...
      <div style={s.formWrap}>
        <div style={s.formCard}>
          ...
          <div style={s.infoBox}>
            ...
            <div style={s.infoItem}><span style={s.infoLabel}>Monthly Return %</span> — Your total % gain or loss from the 1st of this month to today. Update every trading day.</div>
            <div style={s.infoItem}><span style={s.infoLabel}>Win Rate %</span> — Winning trades ÷ total trades × 100. e.g. 14 wins out of 20 = 70%.</div>
            <div style={s.infoItem}><span style={s.infoLabel}>Total Trades</span> — Total closed trades this month.</div>
            <div style={s.infoItem}><span style={s.infoLabel}>Max Drawdown %</span> — Biggest peak-to-trough loss this month. e.g. account dropped 8% from its high = enter 8.</div>
            <div style={s.infoItem}><span style={s.infoLabel}>Best Trade</span> — Optional. Ticker and % gain to show on your profile.</div>
          </div>
          <div style={s.fieldRow}>
            <div style={s.fieldGroup}>
      ...
      <div style={s.page}>
        ...
        <div style={s.dashWrap}>
          <div style={s.profileCard}>
            ...
            {profileTrader.max_drawdown > 0 && (
              <div style={{ fontSize: 13, color: "#888", marginBottom: 12 }}>Max Drawdown: <span style={{ color: "#EB5757" }}>-{profileTrader.max_drawdown}%</span></div>
            )}
            <div style={s.rankLine}>
          ...
          {profileTrader.history && profileTrader.history.length > 1 && (
            <div style={s.card}>
              <div style={s.cardLabel}>RETURN HISTORY</div>
              <MiniChart data={profileTrader.history} width={300} />
              <div style={{ display: "flex", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
                {profileTrader.history.map((v, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: v >= 0 ? "#6FCF97" : "#EB5757" }}>{v > 0 ? "+" : ""}{v}%</div>
                    <div style={{ fontSize: 10, color: "#555570" }}>Update {i + 1}</div>
                  </div>
          ...
          {profileTrader.trades && profileTrader.trades.length > 0 && (
            <div style={s.card}>
              <div style={s.cardLabel}>BEST TRADES THIS MONTH</div>
              {profileTrader.trades.map((t, i) => (
                <div key={i} style={s.tradeRow}>
                  <span style={s.tradeTicker}>{t.ticker}</span>
                  <span style={{ color: t.pct >= 0 ? "#6FCF97" : "#EB5757", fontWeight: 600, fontSize: 15 }}>{t.pct >= 0 ? "+" : ""}{t.pct}%</span>
                </div>
    ...
    <div style={s.page}>
      ...
      <div style={s.lbWrap}>
        <div style={s.lbHeader}>
          ...
          <p style={s.formSub}>Ranked by Sharpe Score — return, win rate, consistency, and risk combined. Click any trader to view their profile.</p>
        </div>
        {myRank && (
        ...
        <div style={s.lbList}>
          {sorted.map((trader, i) => {
            ...
            return (
              <div key={trader.handle} style={{ ...s.lbRow, ...(isMe ? s.lbRowMe : {}) }}
                onClick={() => openProfile(trader)}>
                <div style={s.lbLeft}>
                ...
                <div style={s.lbRight}>
                  ...
                  {trader.history && trader.history.length > 1 && <MiniChart data={trader.history} width={60} />}
                </div>
              </div>
            );
          })}
...
const s = {
  ...
  profileStats: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 12 },

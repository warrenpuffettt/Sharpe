...
export default function Sharpe(){
  ...
  async function handleReceiptUpload(){
    ...
    // Check if entry for this month already exists — if so merge, else insert
    const{data:existing}=await supabase.from("receipts").select("*").eq("handle",currentUser.handle).eq("month",month).maybeSingle();
    if(existing){
      const merged=[...(existing.urls||[]),...uploadedUrls];
      await supabase.from("receipts").update({urls:merged}).eq("id",existing.id);
    } else {
      const{error:dbErr}=await supabase.from("receipts").insert({handle:currentUser.handle,month,urls:uploadedUrls});
      if(dbErr){toast2("Error saving — try again");console.error(dbErr);setReceiptUploading(false);return;}
    }
                ...
                <div>
                  ...
                  {profileReceipts.length===0?(
                    <div style={{...s.card,textAlign:"center",padding:48}}>
                      ...
                      <div style={{fontSize:12,color:"#333348"}}>{isMe?"Upload your first monthly performance card to build credibility":"This trader hasn't uploaded verification yet"}</div>
                    </div>
                  ):(
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {profileReceipts.map((r,i)=>{
                        const isOpen=openMonthGallery===i;
                        const urls=r.urls||[];
                        return(
                          <div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,overflow:"hidden"}}>
                            <div onClick={()=>setOpenMonthGallery(isOpen?null:i)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",cursor:"pointer"}}>
                              <div style={{display:"flex",alignItems:"center",gap:12}}>
                                <span style={{fontSize:20}}>🧾</span>
                                <div>
                                  <div style={{fontSize:13,fontWeight:700,color:"#C8A96E",fontFamily:"'DM Mono',monospace"}}>{r.month||"Performance"}</div>
                                  <div style={{fontSize:11,color:"#444458",marginTop:2}}>{urls.length} image{urls.length!==1?"s":""} · ✓ Verified</div>
                                </div>
                              </div>
                              <span style={{color:"#555570",fontSize:13}}>{isOpen?"▲":"▼"}</span>
                            </div>
                            {isOpen&&urls.length>0&&(
                              <div style={{padding:"0 14px 14px"}}>
                                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
                                  {urls.map((url,j)=>(
                                    <div key={j} onClick={()=>setLightboxImg(url)} style={{borderRadius:8,overflow:"hidden",cursor:"zoom-in",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)"}}>
                                      <img src={url} alt={r.month+" "+(j+1)} style={{width:"100%",display:"block",aspectRatio:"4/3",objectFit:"cover"}}/>
                                    </div>
                                  ))}
                                </div>
                              </div>

  ...
  if(showReceiptModal) return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:"#13131A",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:28,maxWidth:480,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <p style={{fontSize:13,color:"#555570",marginBottom:20,lineHeight:1.65}}>Help your followers verify your track record. This builds trust and unlocks more features.</p>

        <div style={{background:"rgba(111,207,151,0.05)",border:"1px solid rgba(111,207,151,0.2)",borderRadius:10,padding:14,marginBottom:20}}>
          <div style={{fontSize:12,fontWeight:700,color:"#6FCF97",marginBottom:10}}>✓ WHAT TO UPLOAD</div>
          {[["📊","Monthly performance summary card","Webull, Robinhood, TD Ameritrade, etc."],["📈","Trade history screenshot","Show closed trades with % gain/loss per trade"],["💹","PnL overview","Cropped to show % returns only — not dollar amounts"]].map(([icon,title,sub])=>(
            <div key={title} style={{display:"flex",gap:10,marginBottom:8}}>
              <span style={{fontSize:16,flexShrink:0}}>{icon}</span>
              <div><div style={{fontSize:12,color:"#E8E8F0",fontWeight:600}}>{title}</div><div style={{fontSize:11,color:"#555570"}}>{sub}</div></div>
            </div>
          ))}
        </div>

        <div style={{background:"rgba(235,87,87,0.05)",border:"1px solid rgba(235,87,87,0.2)",borderRadius:10,padding:14,marginBottom:20}}>
          <div style={{fontSize:12,fontWeight:700,color:"#EB5757",marginBottom:10}}>✕ NEVER INCLUDE</div>
          {["Account balance in dollars","Full name or personal details","Account number or login info","Social security or tax info"].map(item=>(
            <div key={item} style={{fontSize:12,color:"#888899",marginBottom:5,display:"flex",gap:8}}>
              <span style={{color:"#EB5757",flexShrink:0}}>✕</span>{item}
            </div>
          ))}
        </div>
        ...
        <div style={s.fg}>
          ...
          {receiptUploadFile&&<div style={{fontSize:11,color:"#6FCF97",marginTop:4}}>✓ {receiptUploadFile.name}</div>}
        </div>

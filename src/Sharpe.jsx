...
export default function Sharpe(){
  ...
  async function handleAvatarUpload(file){
    ...
    const mime=ext==="jpg"||ext==="jpeg"?"image/jpeg":ext==="png"?"image/png":ext==="gif"?"image/gif":"image/jpeg";
    const p=currentUser.handle.replace("@","")+"/av_"+Date.now()+"."+ext;
    const{error}=await supabase.storage.from("avatars").upload(p,file,{upsert:true,contentType:mime});
    if(error){toast2("Upload failed: "+error.message);console.error(error);return;}
  ...
  if(view==="feed") return(
    <div style={s.page}><div style={s.grain}/><Nav/>
      <div style={{maxWidth:680,margin:"0 auto",padding:"32px 24px",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {visibleFeed.map((post,i)=><div key={post.id}><PostCard post={post} currentUser={currentUser} onLike={handleLike} onComment={handleComment} onReact={handleReact} comments={comments} traders={traders} onOpen={openProfile}/>{i>0&&(i+1)%8===0&&}</div>)}
